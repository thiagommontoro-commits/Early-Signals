...
Mudanças desta versão em relação à anterior:
  1. Importa e usa o NOVO 'modulo_juros_rural.py', inserindo 2 cards
     (Juros PF e Juros PJ) nos Fatores Econômicos do Brasil.
  2. Prompt da IA exige comparação SEMPRE contra o MÊS ANTERIOR.
  3. Prompt da IA exige que o campo "impactos" cite linhas de máquinas
     (tratores/colheitadeiras/pulverizadores/plantadeiras).
...
def get_direcao_badge(analise, mes_anterior_nome=None):
    """Selo de tendência (direção) do fator SEMPRE comparado ao mês anterior.

    Usa o campo 'direcao' vindo da IA: 'melhorando' | 'piorando' | 'estavel'.
    O rótulo exibido deixa EXPLÍCITO o mês de comparação (ex.: "Melhorando
    vs Agosto"), para que o leitor saiba que a leitura é mês contra mês.
    Se ausente (cache antigo/inadimplência), assume 'estavel' (nunca quebra).
    """
    ...
    # Sufixo "vs <mês anterior>" para deixar a base de comparação explícita.
    if mes_anterior_nome:
        sufixo = {
            "pt": f' vs {mes_anterior_nome["pt"]}',
            "en": f' vs {mes_anterior_nome["en"]}',
            "es": f' vs {mes_anterior_nome["es"]}',
        }
    else:
        sufixo = {"pt": "", "en": "", "es": ""}
...
def gerar_bloco_analises(analises_pais, mes_anterior_nome=None):
    ...
    'mes_anterior_nome' (dict pt/en/es) é usado no selo de direção para
    explicitar que a comparação é contra o mês anterior.
    """
...
def atualizar_dados_com_ia(dados_path, script_dir):
    ...
    1.  `fatores_economicos`: análises macroeconômicas concisas para os fatores listados.
        ...
        - "direcao": Movimento do fator em {mes_nome}/{year} COMPARADO A {mes_anterior_nome}/{ano_mes_anterior}.
        ...
        - "descricao": Análise curta (máx 25 palavras) que DEVE explicitar a comparação
          com {mes_anterior_nome} (use expressões como "ante {mes_anterior_nome}",
          "vs {mes_anterior_nome}", "frente a {mes_anterior_nome}"), citando o número/percentual
          quando houver (ex.: "Selic estável em 15% ante {mes_anterior_nome}").
        - "impactos": **IMPACTO DIRETO E CONCRETO NO MERCADO DE MÁQUINAS AGRÍCOLAS** (máx 30 palavras).
          ...
          em relação a {mes_anterior_nome}.
...
def gerar_dashboard(template_path, output_path, dados_path):
    try:
        ...
        # Mês anterior — usado nos selos de direção para deixar explícito que a
        # comparação dos fatores econômicos é SEMPRE contra o mês anterior.
        mes_anterior_idx = (now.month - 2) % 12
        mes_anterior_nome = {
            "pt": TRANSLATIONS["pt"]["months"][mes_anterior_idx],
            "en": TRANSLATIONS["en"]["months"][mes_anterior_idx],
            "es": TRANSLATIONS["es"]["months"][mes_anterior_idx],
        }
        print(f"📅 Comparação dos fatores econômicos: "
        ...
        for codigo_pais, dados_pais in dados_completos.items():
            ...
            # Gerar Fatores Econômicos (selo de direção explicita "vs <mês anterior>")
        ...
        # A aba "Margem do Produtor" foi retirada do Early Signals. Nada é
        # processado aqui. Benefício adicional: o tempo de execução do Action
        # cai drasticamente, já que este módulo fazia dezenas de consultas
        # sequenciais à CONAB (com rate-limit) a cada geração.
