...
def get_direcao_badge(analise):
    """Selo de tendência (direção projetada) do fator.
    ...
    attrs = f'data-pt="{rot["pt"]}" data-en="{rot["en"]}" data-es="{rot["es"]}"'
    return f'<span class="dir-pill {css}">{seta} <span {attrs}>{rot["pt"]}</span></span>'
...
def gerar_bloco_analises(analises_pais):
    ...
    for analise in analises_pais:
        ...
        direcao_html = get_direcao_badge(analise)
...
def atualizar_dados_com_ia(dados_path, script_dir):
    ...
    1.  `fatores_economicos`: análises macroeconômicas concisas para os fatores listados.
        ...
        - "direcao": Perspectiva do fator para os próximos 1-3 meses, do ponto de vista do mercado de máquinas agrícolas. Use APENAS um destes valores: 'melhorando', 'piorando' ou 'estavel'. Esta é a DIREÇÃO/tendência (para onde o fator caminha), independente do estado atual.
...
def gerar_dashboard(template_path, output_path, dados_path):
    try:
        ...
        # ---- MÓDULO D: MARGEM & CUSTO AGRÍCOLA (CONAB) ----
        print("Processando Margem Agrícola (CONAB)...")

        placeholder_margem = "{{BLOCO_MARGEM}}"
        if placeholder_margem not in html_content:
            raise ValueError(
                "O placeholder {{BLOCO_MARGEM}} não foi encontrado no arquivo "
                "dashboard_template.html. Verifique se a aba de Margem foi adicionada ao template."
            )

        dados_margem = processar_margem_agricola()
        if dados_margem is None:
            raise ValueError("O módulo de Margem Agrícola (processar_margem_agricola) não retornou dados.")

        bloco_margem_html = gerar_bloco_margem(dados_margem)
        if not bloco_margem_html or not bloco_margem_html.strip():
            raise ValueError("O renderizador da Margem Agrícola (gerar_bloco_margem) retornou um bloco HTML vazio.")

        html_content = html_content.replace(placeholder_margem, bloco_margem_html)
        html_content = html_content.replace("</head>", CSS_MARGEM + "</head>")
        print(f"✅ Margem Agrícola inserida no dashboard ({len(bloco_margem_html):,} caracteres).")
