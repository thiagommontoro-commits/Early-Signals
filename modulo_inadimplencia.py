...
# (o dashboard já mapeia essas tendências em get_farol_class())
# ==========================================================================
...
SERIE_SGS = 21148
URL = f"https://api.bcb.gov.br/dados/serie/bcdata.sgs.{SERIE_SGS}/dados/ultimos/24?formato=json"
...
def obter_inadimplencia_rural():
    ...
    # média histórica dos últimos 24 meses (base de comparação)
    ...
    descricao = _multi(
        pt=(f"Crédito rural (PF): {atual:.2f}% em {mes_ref}. "
            f"{seta_pt} {delta_pp:+.2f} p.p. no mês · {pos_pt} 24m ({media_hist:.2f}%)."),
        en=(f"Rural credit (individuals): {atual:.2f}% in {mes_ref}. "
            f"{seta_en} {delta_pp:+.2f} p.p. MoM · {pos_en} 24m ({media_hist:.2f}%)."),
        es=(f"Crédito rural (PF): {atual:.2f}% en {mes_ref}. "
            f"{seta_es} {delta_pp:+.2f} p.p. mes · {pos_es} 24m ({media_hist:.2f}%)."),
    )
