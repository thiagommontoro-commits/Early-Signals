...
def obter_inadimplencia_rural():
    ...
    # DESCRIÇÃO e IMPACTOS — STRING SIMPLES em português.
    # (O dashboard renderiza esses campos como texto simples; enviar um dict
    #  faria aparecer o {'pt':...,'en':...} cru na tela. A tradução da página
    #  é feita pelo widget Google Translate por cima do texto PT.)
    descricao = (f"Crédito rural (PF): {atual:.2f}% em {mes_ref}. "
                 f"{seta_pt} {delta_pp:+.2f} p.p. no mês · {pos_pt} {janela_meses}m ({media_hist:.2f}%).")

    if tendencia == "baixa":
        impactos = "Queda favorece oferta de crédito agro e financiamento de máquinas."
    elif tendencia == "alta":
        impactos = "Alta pode restringir crédito agro e pressionar financiamento de máquinas."
    else:
        impactos = "Estabilidade mantém as condições de crédito para máquinas."
