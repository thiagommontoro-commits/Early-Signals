# -*- coding: utf-8 -*-
"""
render_margem_agricola.py  (v2.1 — com sinalização de PROVENIÊNCIA + robustez a dados incompletos)
====================================================================================================
...
de cada dado:
  ...
  ○ Indisponível -> fonte não retornou dado (ou o cache local está desatualizado
                    e não contém os campos de status) -> "—"

Interface (contrato do gerador):
    from render_margem_agricola import gerar_bloco_margem, CSS_MARGEM

NOTA DE ROBUSTEZ (v2.1):
    Todos os campos de status/proveniência (status_geral, custo_status,
    produtividade_status, preco_status) e valores numéricos passaram a ser
    lidos com `.get(chave, padrao)` em vez de indexação direta `[chave]`.
    Isso evita que um card quebre o dashboard inteiro (KeyError) quando o
    cache local (ex.: cache_margem_AAAA_MM.json) foi gerado por uma versão
    anterior do coletor e não possui todas as chaves esperadas. Nesses
    casos, o ponto correspondente é simplesmente tratado como "incompleto"
    (○ Indisponível), preservando a renderização do restante do dashboard.
...
def _card_html(card):
    ...
    if not serie:
        # Card sem nenhuma série de dados -> renderiza como totalmente indisponível
        serie = [{}]
    vigente = serie[-1]
    ...
    for i, s in enumerate(serie):
        ...
        marca = _mk(s.get("custo_total_ha"), s.get("custo_status", "incompleto"))
        minis += (
