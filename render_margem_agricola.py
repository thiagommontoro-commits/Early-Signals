# -*- coding: utf-8 -*-
"""
render_margem_agricola.py  (v2.4 — proveniência + robustez + GUARD visual de outliers)
========================================================================================
...
de cada dado:
  ...
  ◐ Repetido     -> herdado da safra anterior (carry-forward) — "*" e cor âmbar
  ○ Indisponível -> fonte não retornou dado -> "—"
  ⚠ Suspeito     -> a margem calculada fugiu da faixa plausível para a cultura
                    (possível erro de unidade/conversão na coleta) — o valor
                    NÃO é escondido, mas é destacado em vermelho com um aviso
                    explícito, para nunca ser lido como confiável sem checagem.
...
>>> IMPORTANTE: este é o arquivo COMPLETO da v2.4. Ao atualizar no GitHub,
    ...
    nunca cole apenas um trecho sobre uma versão antiga.

HISTÓRICO DE VERSÕES:
    v2.1 - robustez: leitura via .get() em vez de indexação direta, evitando
           KeyError quando o cache é de schema antigo.
    v2.2 - suporte visual à cultura Café (ícone ☕) + contagem dinâmica de
           combinações no resumo (não fixa mais em "8 culturas").
    v2.3 - (sem mudança de render; cascata de aliases ficou no módulo).
    v2.4 - GUARD DE OUTLIERS: pontos com 'alerta_valor' (vindo do módulo)
           são exibidos com destaque de "⚠ Verificar valor" em vez de serem
           apresentados como um número confiável. Também corrige o asterisco
           "*" para só aparecer quando a MARGEM existe e é derivada de dado
           repetido (evita o "—*" enganoso em safras sem margem).

Autoria: Global Reporting & Analytics — Thiago Montoro (AGCO)
"""

_ICO = {"Soja": "🌱", "Milho": "🌽", "Algodão": "🧵", "Trigo": "🌾", "Café": "☕"}
_BADGE = {
    "alta":         ("pos", "Margem ↑"),
    "baixa":        ("neg", "Margem ↓"),
    "estavel":      ("neu", "Estável"),
...
def _card_html(card):
    ...
    vigente_suspeita = bool(vigente.get("alerta_valor"))
    if vigente_suspeita:
    ...
    for i, s in enumerate(serie):
        margem = s.get("margem_ha")
        status_geral = s.get("status_geral", "incompleto")
        suspeito = bool(s.get("alerta_valor"))
        is_last = (i == len(serie) - 1)
        cell_q = ""
        if suspeito:
            cell_q = " sf-sus"
        elif status_geral == "parcial":
            cell_q = " sf-rep"
        elif status_geral == "incompleto":
            cell_q = " sf-off"
        delta_html = "&nbsp;"
        if i > 0:
            ant = serie[i - 1].get("margem_ha")
        ...
        marca = "*" if (margem is not None and status_geral == "parcial" and not suspeito) else ""
        valor_mg = f'⚠ {_fmt(margem, "")}' if suspeito else f'{_fmt(margem, "")}{marca}'
        titulo_sus = f' title="{s.get("alerta_motivo","")}"' if suspeito else ""
        minis += (
            f'<div class="sf{" latest" if is_last else ""}{cell_q}"{titulo_sus}>'
            f'<div class="yr">{s.get("safra", "—")}</div>'
            f'<div class="mg">{valor_mg}</div>'
    ...
    for s in serie:
        custo_parts.append(
            _fmt(s.get("custo_total_ha"), "") + _mk(s.get("custo_total_ha"), s.get("custo_status", "incompleto"))
        )
    custos = " → ".join(custo_parts)

    produtividade = vigente.get("produtividade")
    prod_txt = "—" if produtividade is None else f'{produtividade:.0f} {unid}'
    prod_txt += _mk(produtividade, vigente.get("produtividade_status", "incompleto"))
    preco_mk = _mk(vigente.get("preco_medio"), vigente.get("preco_status", "incompleto"))
    custo_mk = _mk(vigente.get("custo_total_ha"), vigente.get("custo_status", "incompleto"))

    kpi_valor = f'⚠ {_fmt(vigente.get("margem_ha"))}' if vigente_suspeita else _fmt(vigente.get('margem_ha'))
    aviso_kpi = (f'<div class="mc-alerta">⚠ {vigente.get("alerta_motivo","Valor fora da faixa plausível — verificar coleta.")}</div>'
                 if vigente_suspeita else "")
    ...
    return f"""
      <div class="margem-card {cls_card}">
        ...
        <div class="mc-kpi-label">Margem {vigente.get('safra', '—')}</div>
        <div class="mc-kpi {cls_card}">{kpi_valor}/ha</div>
        {aviso_kpi}
        <div class="mc-sub">Receita <b>{_fmt(vigente.get('receita_ha'))}{preco_mk}</b> ·
             Custo <b>{_fmt(vigente.get('custo_total_ha'))}{custo_mk}</b> ·
...
def gerar_bloco_margem(dados_margem):
    ...
    resumo_html = ""
    if rq:
        safras_list = dados_margem.get("safras", [])
        n_saf = len(safras_list) or 3
        n_comb = dados_margem.get("n_combinacoes") or (rq.get("total", 0) // n_saf if n_saf else 0)
        suspeitos = rq.get("suspeitos", 0)
        suspeitos_html = (
            f'<span class="qp q-sus">⚠ {suspeitos} suspeito(s) — verificar</span>' if suspeitos else ""
        )
        resumo_html = (
            ...
            f'{suspeitos_html}'
            f'<span class="qp-total">de {rq.get("total",0)} pontos '
            f'({n_comb} combinações cultura×UF × {n_saf} safras)</span>'
            f'</div>'
        )
    ...
    legenda_html = (
        ...
        '<span class="lg q-sus">⚠ Suspeito</span> = valor fora da faixa plausível '
        'para a cultura (possível erro de unidade/conversão) — requer checagem manual '
        'antes de ser usado. '
...
.mc-alerta { background:#fce4e4; border-left:4px solid #c00000; color:#8a0000; font-size:10.5px; font-weight:700; padding:6px 10px; border-radius:4px; margin:6px 0 10px; line-height:1.4; }
