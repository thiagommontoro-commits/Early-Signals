# -*- coding: utf-8 -*-
"""
render_margem_agricola.py  (v2.2 — proveniência + robustez + asterisco correto)
================================================================================
...
de cada dado:
  ...
  ○ Indisponível -> fonte não retornou dado (ou cache desatualizado) -> "—"

Interface (contrato do gerador):
    from render_margem_agricola import gerar_bloco_margem, CSS_MARGEM

ROBUSTEZ (v2.1): todos os campos de status/proveniência e valores numéricos são
    lidos com `.get(chave, padrao)` em vez de indexação direta `[chave]`, evitando
    KeyError quando o cache local foi gerado por versão anterior do coletor.

CORREÇÃO COSMÉTICA (v2.2): o asterisco "*" na célula de MARGEM só é exibido quando
    a margem EXISTE e é derivada de insumo repetido (status_geral == "parcial").
    Antes, o "*" era herdado do custo repetido e aparecia como "—*" em safras sem
    margem, o que era enganoso.
...
def _card_html(card):
    ...
    for i, s in enumerate(serie):
        ...
        # v2.2: '*' na MARGEM só quando a margem existe E é derivada de dado repetido.
        marca = "*" if (margem is not None and status_geral == "parcial") else ""
        minis += (
    ...
    return f"""
      <div class="margem-card {cls_card}">
        ...
        <div class="mc-kpi {cls_card}">{_fmt(vigente.get('margem_ha'))}/ha</div>
...
def gerar_bloco_margem(dados_margem):
    ...
    resumo_html = ""
    if rq:
        # Contagem dinâmica de combinações/safras (não fixa mais "8 culturas × 3 safras").
