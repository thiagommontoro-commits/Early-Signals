# -*- coding: utf-8 -*-
"""
render_margem_agricola.py  (v3.0 — Receita Bruta + Margem Bruta + Margem Econômica)
==================================================================================
Renderiza a aba "Margem do Produtor" com TRÊS indicadores por cultura/UF,
alinhados à metodologia de custos da CONAB (COE/COT/CT):

    • Receita Bruta (R$/ha)    = Produtividade x Preço CEPEA
    • Margem Bruta (R$/ha)     = Receita - COE   (o produtor gera caixa?)
    • Margem Econômica (R$/ha) = Receita - CT     (remunera terra e capital?)

Destaques visuais:
  ● Oficial      -> dado veio direto da CONAB/CEPEA para aquela safra
  ◐ Repetido     -> herdado da safra anterior (carry-forward) — "*" âmbar
  ○ Indisponível -> fonte não retornou dado -> "—"
  ⚠ Suspeito     -> margem econômica fora da faixa plausível (destaque vermelho)

Insight executivo — "Aperto Econômico" (Spread):
  Spread = Margem Bruta - Margem Econômica = quanto do resultado é consumido
  por terra, capital e depreciação. Quanto MAIOR o spread em relação à margem
  bruta, mais pressionada a decisão de investir em máquinas — mesmo com caixa
  positivo. É um sinal antecedente forte de demanda AGCO.

Interface (contrato do gerador):
    from render_margem_agricola import gerar_bloco_margem, CSS_MARGEM

>>> Substitua TODO o conteúdo do arquivo ao atualizar no GitHub.
...
def _card_html(card):
    ...
    # Deltas ano-a-ano (vs safra anterior)
    ...
    # Mini-histórico (2 safras) para cada indicador
    def _mini(chave):
        cells = ""
        for i, s in enumerate(serie):
            ...
            cq = " sf-sus" if sus else (" sf-off" if v is None else (" latest" if is_last else ""))
...
def gerar_bloco_margem(dados_margem):
    ...
    legenda_html = (
        ...
        'comparativo é ano-a-ano (2 safras).</i>'
        '</div>'
    )
...
CSS_MARGEM = """
<style>
/* ===== EARLY SIGNALS · Aba Margem do Produtor v3.0 (CONAB COE/COT/CT) ===== */
...
/* Grid auto-fit — idêntico ao padrão das abas Commodities/Crédito, acomoda
   muitas culturas mantendo o mesmo tamanho de cartão em qualquer quantidade. */
