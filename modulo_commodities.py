...
def _tendencia_futura(serie):
    ...
    Combina dois sinais:
      1) MOMENTUM: variação % média dos últimos 3 meses (peso 60%)
      2) SAZONALIDADE: como o próximo mês historicamente se comporta vs o mês
         atual, na média de todos os anos da série (peso 40%)
    Retorna dict: {classe, rotulo_pt/en/es, seta, forca(%)}
    """
    if len(serie) < 4:
        return {"classe": "estavel", "seta": "▬",
                "pt": "Estável", "en": "Stable", "es": "Estable", "forca": 0.0}

    # 1) MOMENTUM: média das variações % dos últimos 3 meses
    ult = [v for (_, _, v) in serie[-4:]]  # 4 pontos -> 3 variações
    variacoes = []
    ...
    # 2) SAZONALIDADE: média histórica da variação (mês_atual -> mês seguinte)
    _, mes_atual, _ = serie[-1]
    mes_seguinte = mes_atual % 12 + 1
    # coleta pares (mês_atual -> mês_seguinte) ao longo dos anos
    por_mes = {}
    ...
    # combina (momentum 60% + sazonalidade 40%)
    score = 0.6 * momentum + 0.4 * sazonalidade

    if score > 1.0:
        return {"classe": "positivo", "seta": "▲",
                "pt": "Tendência de alta", "en": "Upward trend", "es": "Tendencia alcista",
                "forca": round(score, 1)}
    if score < -1.0:
        return {"classe": "negativo", "seta": "▼",
                "pt": "Tendência de baixa", "en": "Downward trend", "es": "Tendencia bajista",
                "forca": round(score, 1)}
    return {"classe": "estavel", "seta": "▬",
            "pt": "Tendência estável", "en": "Stable trend", "es": "Tendencia estable",
            "forca": round(score, 1)}
