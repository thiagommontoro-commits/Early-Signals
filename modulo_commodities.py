...
COMMODITIES = {
    ...
    # ----------------------------------------------------------------------
    # NOVAS CULTURAS (Laranja e Cana-de-açúcar).
    # Não existe cotação internacional CBOT/ICE equivalente para elas no
    # arquivo, então usamos as séries domésticas da própria planilha.
    # "coluna": None  ->  a coluna é LOCALIZADA PELO CABEÇALHO (chaves/excluir),
    # varrendo as abas de "abas_candidatas". Se não encontrar, o card
    # simplesmente NÃO é gerado (nenhum valor estimado ou inventado).
    # ----------------------------------------------------------------------
    "laranja": {"aba": "Demais AGRÍCOLAS", "coluna": None, "unidade": "R$/40,8 kg", "icone": "🍊",
                "abas_candidatas": ["Demais AGRÍCOLAS", "Demais Agrícolas", "Laranja", "Citros"],
                "chaves": ["LARANJA", "INDUSTRIA"],
                "excluir": ["MESA"],
                "fonte": "CEPEA/ESALQ - Laranja Indústria (SP)",
                "nome": {"pt": "Laranja Indústria (SP)", "en": "Orange, Processing (SP)",
                         "es": "Naranja Industria (SP)"},
                "regionais": []},
    "cana":    {"aba": "Cana",             "coluna": None, "unidade": "R$/t", "icone": "🎋",
                "abas_candidatas": ["Cana", "Demais AGRÍCOLAS", "Demais Agrícolas"],
                "chaves": ["CANA"],
                "excluir": ["ICE", "ACUCAR", "AZUCAR", "ETANOL", "ANIDRO", "HIDRATADO"],
                "fonte": "CONSECANA/CEPEA - Cana-de-açúcar (SP)",
                "nome": {"pt": "Cana-de-açúcar (SP)", "en": "Sugarcane (SP)",
                         "es": "Caña de azúcar (SP)"},
                "regionais": []},
}
...
def _norm(txt):
    """Normaliza texto de cabeçalho: sem acento, maiúsculo, espaços colapsados."""
    if txt is None:
        return ""
...
def _mapa_cabecalhos(ws, row_end=6, col_end=80):
    """Concatena as linhas de cabeçalho (1..row_end) de cada coluna."""
    mapa = {}
    for c in range(3, col_end + 1):
        partes = [_norm(ws.cell(row=r, column=c).value) for r in range(1, row_end + 1)]
...
def _localizar_coluna(ws, chaves, excluir=None):
    """
    Retorna o índice da 1ª coluna cujo cabeçalho contém TODAS as 'chaves'
    e NENHUM dos termos de 'excluir'. Retorna None se não encontrar.
    """
    excluir = [_norm(e) for e in (excluir or [])]
    chaves = [_norm(k) for k in chaves]
    for col, texto in sorted(_mapa_cabecalhos(ws).items()):
        if all(k in texto for k in chaves) and not any(e in texto for e in excluir):
            return col
    return None


def _resolver_fonte(cfg, wb, cid):
    """
    Devolve (worksheet, coluna) para a commodity.
    - Coluna fixa (mapeamento validado): usa direto.
    - Coluna None: procura pelo cabeçalho nas abas candidatas.
    """
    if cfg.get("coluna"):
        if cfg["aba"] not in wb.sheetnames:
            return None, None
        return wb[cfg["aba"]], cfg["coluna"]

    for nome_aba in cfg.get("abas_candidatas", [cfg["aba"]]):
        if nome_aba not in wb.sheetnames:
            continue
        ws_cand = wb[nome_aba]
        col = _localizar_coluna(ws_cand, cfg.get("chaves", []), cfg.get("excluir"))
        if col:
            print(f"   🔎 {cid}: coluna localizada por cabeçalho -> aba '{nome_aba}', coluna {col}")
            return ws_cand, col
    print(f"   ⚠️ {cid}: série não localizada na planilha (card não será gerado).")
    return None, None
...
def calcular_commodity(serie, cfg, ws=None):
    ...
    return {
        ...
        "fonte": cfg.get("fonte", FONTE_PUBLICA),
    }
...
def processar_commodities(xlsx_path):
    ...
    for cid, cfg in COMMODITIES.items():
        try:
            ws, col = _resolver_fonte(cfg, wb, cid)
            if ws is None or col is None:
                continue
            serie = _extrair_serie(ws, col)
