# -*- coding: utf-8 -*-
"""
modulo_margem_agricola.py  (v3.6 — override de preço histórico p/ safra fechada)
===========================================================================
...
>>> CORREÇÃO v3.6 (override de preço histórico — safra FECHADA):
    ...
    Ordem de prioridade do preço: (1) override manual; (2) série ampla CEPEA;
    (3) fallback estreito. Nada é inventado: sem preço em nenhuma fonte -> None.
...
SCHEMA_VERSION = "3.6"  # v3.6: override de preço histórico (precos_historicos.json)
...
# ---------------------------------------------------------------------------
# Causa raiz: _preco_medio() consultava o CEPEA com uma janela ESTREITA por
# safra. Para a safra VIGENTE (janela recente) a fonte respondia; para a safra
# ANTERIOR (janela > 1 ano no passado) a fonte servia janela móvel e devolvia
# vazio -> preco=None -> receita/margem=None -> comparativo 24/25 sumia.
#
# Solução: UMA única consulta ampla cobrindo TODAS as safras e, em seguida,
# média por janela de cada safra fatiada LOCALMENTE pela coluna de data.
# Nunca inventa: se a janela ficar sem cotações, cai no fallback estreito e,
# se ainda assim vazio, retorna None para aquela safra.
...
# ---------------------------------------------------------------------------
# v3.6 — OVERRIDE DE PREÇO HISTÓRICO (arquivo precos_historicos.json)
# ---------------------------------------------------------------------------
...
#   { "soja": {"2024/25": 128.50, "2025/26": null}, "milho": {...}, ... }
# Regras: valor numérico -> usado; null/ausente -> ignora (segue coleta).
...
def _preco_manual(overrides, produto, safra):
    """Retorna o preço manual para (produto, safra), tentando cada alias do
    produto. Aceita chaves exatas e em minúsculas. Ignora não-numéricos."""
    if not overrides:
        return None
...
async def _precos_por_safra(cepea, produto, safras, overrides=None):
    """Retorna {safra: preco_medio ou None}, na seguinte ordem de prioridade:
      1) OVERRIDE manual (precos_historicos.json) — preço oficial CEPEA que o
         usuário informa para safras FECHADAS (contorna o 403 em produção);
      2) SÉRIE AMPLA do CEPEA (uma consulta) fatiada por janela set->ago;
      3) FALLBACK estreito por safra (_preco_medio).
    ...
    # (1) override manual primeiro
    for s in safras:
        pm = _preco_manual(overrides, produto, s)
        if pm is not None:
            resultado[s] = pm
            print(f"      ✅ preço {s} via override histórico (precos_historicos.json): R$ {pm}")

    faltantes = [s for s in safras if resultado[s] is None]
    if not faltantes:
        return resultado

    # (2) série ampla do CEPEA — apenas para as safras ainda faltantes
    candidatos = _como_lista(produto)
...
async def _montar_base_async():
    ...
    # v3.6: carrega override de preços históricos (opcional).
    ...
    for combo in COMBINACOES:
        ...
        # v3.5/3.6: preços de TODAS as safras (override -> série ampla -> fallback).
        precos_safra = await _precos_por_safra(cepea, combo["cepea"], safras, _OVERRIDES_PRECO)
        for safra in safras:
...
if __name__ == "__main__":
    print("=" * 64)
    print("EARLY SIGNALS · modulo_margem_agricola v3.6 (teste standalone)")
    print("=" * 64)
