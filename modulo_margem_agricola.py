...
# >>> v2.5: filtro anti-duplicidade de custo. A planilha da CONAB devolve, na
# MESMA coluna 'valor_ha', linhas de DETALHE (ex.: "8 - Sementes") e linhas de
# SUBTOTAL (ex.: "TOTAL DE DEPRECIACOES (E)", "CUSTO FIXO (E+F=G)"). Somar as
# duas conta parte do custo 2x. Uma linha de detalhe SEMPRE comeca com codigo
# numerico ("8 -", "3.1 -", "16.1 -"); subtotais comecam com texto.
import re as _re
_PADRAO_ITEM_DETALHE = _re.compile(r"^\s*\d+(\.\d+)?\s*-")
...
# >>> v2.6: fator caroco->pluma do algodao. A CONAB reporta algodao EM CAROCO
# (~4.911 kg/ha em MT), mas o preco CEPEA e da PLUMA (~40% do peso). Sem isso a
# receita do algodao fica ~2,5x superestimada. Graos = 1.0.
_FATOR_PLUMA_ALGODAO = 0.40


COMBINACOES = [
    {"cultura": "Soja",    "uf": "MT", "conab": "soja",    "cepea": "soja",          "unid": "sc60",   "sc_por_t": 16.667, "fator_comercial": 1.0},
    {"cultura": "Soja",    "uf": "PR", "conab": "soja",    "cepea": "soja",          "unid": "sc60",   "sc_por_t": 16.667, "fator_comercial": 1.0},
    {"cultura": "Soja",    "uf": "RS", "conab": "soja",    "cepea": "soja",          "unid": "sc60",   "sc_por_t": 16.667, "fator_comercial": 1.0},
    {"cultura": "Soja",    "uf": "GO", "conab": "soja",    "cepea": "soja",          "unid": "sc60",   "sc_por_t": 16.667, "fator_comercial": 1.0},
    {"cultura": "Milho",   "uf": "MT", "conab": "milho",   "cepea": "milho",         "unid": "sc60",   "sc_por_t": 16.667, "fator_comercial": 1.0},
    {"cultura": "Milho",   "uf": "PR", "conab": "milho",   "cepea": "milho",         "unid": "sc60",   "sc_por_t": 16.667, "fator_comercial": 1.0},
    {"cultura": "Algodão", "uf": "MT", "conab": "algodao", "cepea": "algodao",       "unid": "arroba", "sc_por_t": 66.667, "fator_comercial": _FATOR_PLUMA_ALGODAO},
    {"cultura": "Trigo",   "uf": "PR", "conab": "trigo",   "cepea": "trigo",         "unid": "sc60",   "sc_por_t": 16.667, "fator_comercial": 1.0},
    # --- Café (saca de 60 kg beneficiado), com CASCATA de aliases (v2.3). ---
    {"cultura": "Café", "uf": "MG",
     "conab": ["cafe", "cafe_arabica", "arabica"],
     "cepea": ["cafe_arabica", "cafe", "arabica"],
     "unid": "sc60", "sc_por_t": 16.667, "fator_comercial": 1.0},
    {"cultura": "Café", "uf": "ES",
    ...
    for i, nome in enumerate(candidatos):
        try:
            ...
            if df is None or len(df) == 0 or "valor_ha" not in getattr(df, "columns", []):
                ...
                continue
            # v2.5: soma SOMENTE linhas de detalhe (exclui subtotais duplicados)
            if "item" in getattr(df, "columns", []):
                mascara = df["item"].apply(_eh_linha_de_detalhe)
                if int(mascara.sum()) == 0:
                    print(f"      ⚠ custo: nenhuma linha de detalhe reconhecida "
                          f"('{nome}' {uf}/{safra}) — usando soma bruta.")
                    df_soma = df
                else:
                    n_sub = len(df) - int(mascara.sum())
                    if n_sub > 0:
                ...
                df_soma = df
            total = float(df_soma["valor_ha"].dropna().sum())
            if total > 0:
                if i > 0:
...
async def _produtividade_sc_ha(conab, cultura_conab, uf, safra, sc_por_t, fator_comercial=1.0):
    ...
    v2.6: 'fator_comercial' converte a produtividade FÍSICA da CONAB para a
    unidade COMERCIAL do CEPEA (grãos=1.0; algodão≈0.40 caroço->pluma)."""
    candidatos = _como_lista(cultura_conab)
    for i, nome in enumerate(candidatos):
        try:
            ...
            if fator_comercial != 1.0:
                print(f"      ℹ produtividade ajustada (fator {fator_comercial}, ex.: caroço->pluma) "
