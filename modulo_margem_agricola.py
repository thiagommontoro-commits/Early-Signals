...
SCHEMA_VERSION = "3.5"  # v3.5: preço por safra via série ampla (corrige safra anterior null)
...
async def _precos_por_safra(cepea, produto, safras):
    """Retorna {safra: preco_medio ou None} usando uma única série ampla.

    Estratégia:
      1) Consulta o CEPEA UMA vez, do início da safra mais antiga até o fim da
         mais recente, tentando cada alias do produto.
      2) Normaliza colunas de data/valor e calcula a média dentro da janela
         (set->ago) de CADA safra.
      3) Fallback: se a série ampla não vier ou uma janela ficar vazia, tenta a
         consulta estreita antiga (_preco_medio) para aquela safra.
    """
    import pandas as pd
    ...
    if df_ok is None or len(df_ok) == 0:
        # Sem série ampla: mantém comportamento antigo por safra.
        for s in safras:
            resultado[s] = await _preco_medio(cepea, produto, s)
        return resultado
    ...
    for s in safras:
        ...
        if len(janela):
            ...
            # Janela vazia na série ampla -> tenta consulta estreita direta.
            preco_fb = await _preco_medio(cepea, produto, s)
            if preco_fb is None:
                print(f"      ↳ preço indisponível para {s} (sem cotações na janela).")
...
async def _montar_base_async():
    ...
    for combo in COMBINACOES:
        ...
        # v3.5: preços de TODAS as safras numa única série ampla (corrige o
        # preço null da safra anterior que zerava o comparativo a/a).
...
if __name__ == "__main__":
    print("=" * 64)
    print("EARLY SIGNALS · modulo_margem_agricola v3.5 (teste standalone)")
    print("=" * 64)
