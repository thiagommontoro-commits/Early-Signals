# -*- coding: utf-8 -*-
"""
modulo_margem_agricola.py  (v3.7.1 — hotfix: override robusto, sem crash)
===========================================================================
...
>>> v3.4: comparativo a/a separado da remoção de cartões por alerta.
>>> v3.5: preço por safra via UMA série ampla, fatiada por janela set->ago.
>>> v3.6: override de preço histórico (precos_historicos.json) p/ safra FECHADA
          (em produção o CEPEA responde 403 e o fallback só traz cotações recentes).
>>> v3.7: override específico por UF quando o indicador é regional (ex.: Milho MT/PR).
>>> v3.7.1 (HOTFIX): _preco_manual passou a devolver SEMPRE uma tupla
          (valor, origem). Na v3.7 havia um retorno solto `return None` para
          overrides vazio, o que fazia `pm, origem = _preco_manual(...)` rebentar
          com "cannot unpack None" quando o precos_historicos.json não era
          encontrado — derrubando TODO o cálculo para a estrutura vazia
          ("Dados de margem indisponíveis"). Agora é à prova de ausência do
          arquivo e de qualquer exceção na resolução do override.
...
SCHEMA_VERSION = "3.7.1"  # hotfix: override robusto (sem crash quando JSON ausente)
...
# ---------------------------------------------------------------------------
# v3.5 — série ampla p/ fatiar por safra
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# v3.6/v3.7/v3.7.1 — OVERRIDE DE PREÇO HISTÓRICO (precos_historicos.json)
# ---------------------------------------------------------------------------
# Estrutura (chave = alias 'cepea' do combo):
#   {
#     "soja":  {"2024/25": 132.72, "2025/26": null},
#     "milho": {
#         "2024/25": null, "2025/26": null,
#         "_uf": { "MT": {"2024/25": 56.08}, "PR": {"2024/25": 66.62} }
#     }
#   }
# Prioridade: (1) overrides[prod]["_uf"][uf][safra]; (2) overrides[prod][safra].
# Valor numérico -> usado; null/ausente -> ignora (segue coleta automática).
    ...
    if p.exists():
        try:
            ...
            if isinstance(dados, dict):
                return dados
            print("   ⚠ precos_historicos.json não é um objeto JSON; ignorando override.")
        except Exception as e:
...
def _preco_manual(overrides, produto, safra, uf=None):
    """Resolve o preço manual para (produto, safra[, uf]).

    Retorna SEMPRE uma tupla (valor, origem):
      - (float, "regional")  -> veio de overrides[produto]["_uf"][uf][safra]
      - (float, "generico")  -> veio de overrides[produto][safra]
      - (None, None)         -> não há override aplicável

    HOTFIX v3.7.1: garante retorno em tupla em TODOS os caminhos (inclusive
    overrides vazio/None e exceções), para o desempacotamento no chamador
    `pm, origem = _preco_manual(...)` nunca rebentar.
    """
    try:
        if not overrides or not isinstance(overrides, dict):
            return None, None
        for nome in _como_lista(produto):
            bloco = overrides.get(nome)
            if bloco is None:
                bloco = overrides.get(str(nome).lower())
            if not isinstance(bloco, dict):
                continue
            # (1) override regional específico por UF
            if uf:
                bloco_uf = bloco.get("_uf")
                if isinstance(bloco_uf, dict):
                    sub = bloco_uf.get(uf)
                    if sub is None:
                        sub = bloco_uf.get(str(uf).upper())
                    if isinstance(sub, dict):
                        v = sub.get(safra)
                        if isinstance(v, (int, float)) and not isinstance(v, bool):
                            return float(v), "regional"
            # (2) override genérico da cultura
            v = bloco.get(safra)
            if isinstance(v, (int, float)) and not isinstance(v, bool):
                return float(v), "generico"
    except Exception as e:
        print(f"      ⚠ override de preço ignorado por erro inesperado ({produto}/{safra}): {e}")
    return None, None


async def _precos_por_safra(cepea, produto, safras, overrides=None, uf=None):
    """Retorna {safra: preco_medio ou None}, por prioridade:
      1) OVERRIDE regional (por UF);  2) OVERRIDE genérico da cultura;
      3) SÉRIE AMPLA do CEPEA fatiada por janela;  4) FALLBACK estreito.
    ...
    # (1)+(2) override manual — SEMPRE seguro (retorna tupla)
    for s in safras:
    ...
    # (3) série ampla do CEPEA — apenas para as safras faltantes
    candidatos = _como_lista(produto)
...
async def _montar_base_async():
    ...
    if _OVERRIDES_PRECO:
        n_chaves = len([k for k in _OVERRIDES_PRECO.keys() if not str(k).startswith("_")])
        print(f"   🗂 Override de preços históricos carregado (precos_historicos.json) — {n_chaves} cultura(s).")
    else:
        print("   ℹ Sem precos_historicos.json aplicável; usando apenas coleta automática.")
...
if __name__ == "__main__":
    print("=" * 64)
    print("EARLY SIGNALS · modulo_margem_agricola v3.7.1 (teste standalone)")
    print("=" * 64)
